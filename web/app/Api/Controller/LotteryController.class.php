	...
	function getconfigs($apiparam = array())
	{
		...
		foreach ($_membergrouplists as $k => $v) {
			...
			foreach ($getplayers as $k0 => $v0) {
				$v['min_' . $v0['playid']] = $_xesetconfigs['min_' . $v0['playid']] ?? 0;
				$v['max_' . $v0['playid']] = $_xesetconfigs['max_' . $v0['playid']] ?? 0;
			}
		...
		foreach ($_REQUEST['orderList'] as $k => $v) {
			...
			if (intval($rates[$v['playid']]['maxxf']) > 0) {
				...
				if ($v['fandian']) {
					...
					//$b = intval($v['rate'] * 100);
					if ($a != $b) {
						$this->ajaxReturn(['sign' => false, 'message' => "赔率非法1"]);
						exit;
					}
				} elseif ($v['rate'] != $rate) {
					$this->ajaxReturn(['sign' => false, 'message' => "赔率非法"]);
					exit;
				}
			}
		...
		foreach ($_REQUEST['orderList'] as $k => $v) {
			...
			if (in_array($typeid, $type_fanyong) && $userinfo['parentid'] != '0' && $agentCommissionOpen) {
				...
				foreach ($this->_parent as $k => $v) {
					...
					$fuddetaildata['remark']       = $remark ?? $userinfo['username'] . '下注-佣金发放通过';

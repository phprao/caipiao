...
class ApijiekouController extends \Common\Controller\ApijiekouController
{
	...
	{
		if (!IS_POST) {
			echo jsonreturn(['sign' => false, 'message' => '非法操作']);
			exit;
		}
		$Result = islogin();
		unset($Result['auth']);
	...
	function gettouzhuinfo()
	{
		...
		if (!$sessionid || !$auth_id || !$userinfo) {
			echo jsonreturn(['sign' => false, 'message' => '未登陆', 'data' => ['islogin' => 0]]);
			exit;
		}
		$apiparam = array();
	...
	public function getLotterycode()
	{
		...
		$getLottery = parent::getLotteryCodeCommon($apiparam);
	...
	function checkuername()
	{
		$username = I('username');
		if (!$username) {
			echo jsonreturn(['sign' => false, 'message' => '用户名不能空']);
			exit;
		}
		$apiparam = array();
	...
	function lotterytimes()
	{
		...
		if (!$lotteryname || !$cpinfo) {
			echo jsonreturn(['sign' => false, 'message' => '彩种标识名称不得空']);
			exit;
		}
		//$shortName = I('lotteryname');;
		if (!$lotteryname) {
			echo jsonreturn(['sign' => false, 'message' => '参数错误']);
			exit;
		}

		if ($cpinfo['isopen'] != 1) {
			echo jsonreturn(['sign' => false, 'message' => '彩种已关闭']);
			exit;
		}
		$_classfile = COMMON_PATH . 'Lib/lotterytimes/' . $lotteryname . '.class.php';
		//echo $_classfile;exit;
		if (!is_file($_classfile)) {
			echo jsonreturn(['sign' => false, 'message' => '开奖时间错误']);
			exit;
		}
		$_lotterytimesclass = "Lib\\lotterytimes\\{$lotteryname}";
		//echo $_lotterytimesclass;exit;
	...
	function loadopencode()
	{
		...
		if (!$lotteryname || !$expect) {
			echo jsonreturn(['sign' => false, 'message' => '彩种标识名称或期号不得空']);
			exit;
		}
		$apiparam = array();
		$apiparam['lotteryname'] = $lotteryname;
	...
	function betslisttoday()
	{
		...
		if (!$_REQUEST['lotteryname']) {
			echo jsonreturn(['sign' => false, 'message' => '缺少彩种标识或投注期号']);
			exit;
		}
		...
		if ($Result['data']['islogin'] != 1) {
			echo jsonreturn(['sign' => false, 'message' => '登陆异常，请重新登陆', 'data' => ['islogin' => 0]]);
			exit;
		}
		$apiparam = array();
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/betslisttoday', $apiparam);
		echo jsonreturn($Result);
	}
	...
	{
		if (!IS_POST) {
			$this->ajaxReturn(['sign' => false, 'message' => '非法操作']);
			exit;
		}
		$sessionid = session('member_sessionid');
		$auth_id   = session('member_auth_id');
		if (!$_REQUEST['expect'] || !$_REQUEST['lotteryname']) {
			$this->ajaxReturn(['sign' => false, 'message' => '缺少彩种标识或投注期号']);
			exit;
		}
		if (count($_REQUEST['orderList']) < 1) {
			$this->ajaxReturn(['sign' => false, 'message' => '无投注注单']);
			exit;
		}
		$userinfo = session('userinfo');
		if (!$sessionid || !$auth_id || !$userinfo) {
			$this->ajaxReturn(['sign' => false, 'message' => '未登陆', 'data' => ['islogin' => 0]]);
			exit;
		}
		$Result = [];
		$Result = islogin();
		...
		if ($Result['data']['islogin'] != 1) {
			$this->ajaxReturn(['sign' => false, 'message' => '登陆异常，请重新登陆', 'data' => ['islogin' => 0]]);
			exit;
		}
		if ($Result['data']['islock'] == 1) {
			$this->ajaxReturn(['sign' => false, 'message' => '您的账号不允许下注，请联系客服解锁', 'data' => ['islogin' => 0]]);
			exit;
		}
		...
		foreach ($_REQUEST['orderList'] as $k => $v) {
			if (!$v['playid']) {
				$this->ajaxReturn(['sign' => false, 'message' => $v['playtitle'] . '缺少玩法参数或玩法无法识别']);
				exit;
			}
			if (intval($v['minxf']) > 0 && $v['price'] < $v['minxf']) {
				$this->ajaxReturn(['sign' => false, 'message' => $v['playtitle'] . '最低投注金额为' . $v['minxf']]);
				exit;
			}
			$minxf = $maxxf = 0;
			...
			if (intval($minxf) > 0 && $v['price'] < $minxf) {
				$this->ajaxReturn(['sign' => false, 'message' => $v['playtitle'] . '最低投注金额为' . $minxf]);
				exit;
			}
			if (intval($maxxf) > 0 && $v['price'] > $maxxf) {
				$this->ajaxReturn(['sign' => false, 'message' => $v['playtitle'] . '最高投注金额为' . $maxxf]);
				exit;
			}

			if (intval($v['zhushu']) <= 0) {
				$this->ajaxReturn(['sign' => false, 'message' => $v['playtitle'] . '投注注数错误']);
				exit;
			}
			if (intval($v['totalzs']) <= 0) {
				$this->ajaxReturn(['sign' => false, 'message' => $v['playtitle'] . "系统参数[总注数]设置错误"]);
				exit;
			}
			if (intval($v['zhushu']) > intval($v['totalzs'])) {
				$this->ajaxReturn(['sign' => false, 'message' => $v['playtitle'] . "最高{$v['totalzs']}注"]);
				exit;
			}

			if (strstr($_REQUEST['lotteryname'], 'k3')) {
				if (count(explode('#', $v['number'])) != intval($v['zhushu'])) {
					$this->ajaxReturn(['sign' => false, 'message' => $v['playtitle'] . "-系统检测到您的投注注数异常1"]);
					exit;
				}
		...
		if ($userinfo['balance'] < $totalprice) {
			$this->ajaxReturn(['sign' => false, 'message' => "账户余额不足"]);
			exit;
		}
		$apiparam = array();
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Lottery/cpbuy', $apiparam);
	...
	function defaultuserbankcard()
	{
		...
		$id     = I('id');
		if (!$id) {
			echo jsonreturn(['sign' => false, 'message' => '缺少银行ID参数']);
			exit;
		}
		$sessionid     = session('member_sessionid');
		$auth_id       = session('member_auth_id');
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/defaultuserbankcard', $apiparam);
		$return = [];
	...
	function usergetbankcard()
	{
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/usergetbankcard', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
	...
	function userbindbankcard()
	{
		...
		if (strlen($bankCode) < 1) {
			echo jsonreturn(['sign' => false, 'message' => '请选择银行']);
			exit;
		}
		$pat = '/^\d{16,19}$/';
		if (!preg_match($pat, $bankCardNumber)) {
			echo jsonreturn(['sign' => false, 'message' => '请输入16~19位银行卡账号']);
			exit;
		}
		if ($bankCardNumber != $regbankCardNumber) {
			echo jsonreturn(['sign' => false, 'message' => '银行卡账号两次输入不一致']);
			exit;
		}
		if (strlen($bankBranch) < 12) {
			echo jsonreturn(['sign' => false, 'message' => '请输入正确的开户行网点']);
			exit;
		}
		if (strlen($bankAddress) < 3) {
			echo jsonreturn(['sign' => false, 'message' => '请选择开户行地址']);
			exit;
		}
		if (md5(sha1($bankTradPwd)) != $userinfo['tradepassword'] && $userinfo['tradepassword']) {
			echo jsonreturn(['sign' => false, 'message' => '提款密码错误']);
			exit;
		}
		$apiparam = [];
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/userbindbankcard', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
		echo jsonreturn($return);
	...
	function bankcardList()
	{
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/bankcardList', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
		$return['banklist'] = $Result['banklist'];
	...
	function userbindemail()
	{
		...
		if ($userinfo['email']) {
			echo jsonreturn(['sign' => false, 'message' => '邮箱已经绑定无需重复绑定']);
			exit;
		}
		$myreg = '/^(\w-*\.*)+@(\w-?)+(\.\w{2,})+$/';
		///^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((.[a-zA-Z0-9_-]{2,3}){1,2})$/
		if (!preg_match($myreg, $email)) {
			echo jsonreturn(['sign' => false, 'message' => '邮箱格式错误']);
			exit;
		}
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/userbindemail', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
		echo jsonreturn($return);
	...
	function userbindphone()
	{
		...
		if ($userinfo['tel']) {
			echo jsonreturn(['sign' => false, 'message' => '手机号已经绑定无需重复绑定']);
			exit;
		}
		$myreg = '/^(((13[0-9]{1})|(15[0-9]{1})|(17[0-9]{1})|(18[0-9]{1}))+\d{8})$/';
		if (!preg_match($myreg, $phone)) {
			echo jsonreturn(['sign' => false, 'message' => '手机号码格式错误']);
			exit;
		}
		if (md5(sha1($tradePwd)) != $userinfo['tradepassword'] && $userinfo['tradepassword']) {
			echo jsonreturn(['sign' => false, 'message' => '提款密码错误']);
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/userbindphone', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
		echo jsonreturn($return);
	...
	function changeeditquestion()
	{
		...
		if (!$userinfo['question']) {
			echo jsonreturn(['sign' => false, 'message' => '未设置密保']);
			exit;
		}
		$return = [];
		$return['sign'] = true;
	...
	function questionanscheck()
	{
		...
		if (!is_array($userinfo['question'])) {
			echo jsonreturn(['sign' => false, 'message' => '您未设置密保']);
			exit;
		}
		if (!$answerOne || !$answerTwo || !$answerThree) {
			echo jsonreturn(['sign' => false, 'message' => '密保答案不完整']);
			exit;
		}
		if (md5(sha1($answerOne)) == $userinfo['question']['answerone'] && md5(sha1($answerTwo)) == $userinfo['question']['answertwo'] && md5(sha1($answerThree)) == $userinfo['question']['answerthree']) {
			...
			$_api = new \Lib\api;
			$Result = [];
			$Result = $_api->sendHttpClient('Api/Account/questionanscheck', $apiparam);
			if ($Result['sign'] == true) {
				//$_t = time();
				//$questionanscheck['quetoken'] = md5($_t);
				//$questionanscheck['quetime']  = $_t;
				session('questionanscheck', $Result['questionanscheck']);
				echo jsonreturn(['sign' => true, 'message' => '验证成功', 'questoken' => $Result['questionanscheck']['quetoken']]);
				exit;
			} else {
				echo jsonreturn($apiparam);
				exit;
			...
			echo jsonreturn(['sign' => false, 'message' => '密保答案验证失败']);
			exit;
		}
	}
	function usersecurity()
	{
		...
		if ($questoken) {
			...
			if ($questoken != $questionanscheck['quetoken']) {
				echo jsonreturn(['sign' => false, 'message' => '密保重置token验证失败']);
				exit;
			}
			$_t = time();
			if ($_t - $questionanscheck['quetime'] > 300) {
				echo jsonreturn(['sign' => false, 'message' => '密保重置token验证超时']);
				exit;
			}
		}
		$sessionid     = session('member_sessionid');
		...
		if (!$questionOne) {
			echo jsonreturn(['sign' => false, 'message' => '请选择密保问题1！']);
			exit;
		}
		if (!$questionTwo) {
			echo jsonreturn(['sign' => false, 'message' => '请选择密保问题2！']);
			exit;
		}
		if (!$questionThree) {
			echo jsonreturn(['sign' => false, 'message' => '请选择密保问题3！']);
			exit;
		}
		if (!$answerOne) {
			echo jsonreturn(['sign' => false, 'message' => '请填写密保答案1！']);
			exit;
		}
		if (!$answerTwo) {
			echo jsonreturn(['sign' => false, 'message' => '请填写密保答案2！']);
			exit;
		}
		if (!$answerThree) {
			echo jsonreturn(['sign' => false, 'message' => '请填写密保答案3！']);
			exit;
		}
		if ($questionOne == $questionTwo) {
			echo jsonreturn(['sign' => false, 'message' => '密保问题1和密保问题2不能相同！']);
			exit;
		}
		if ($questionOne == $questionThree) {
			echo jsonreturn(['sign' => false, 'message' => '密保问题1和密保问题3不能相同！']);
			exit;
		}
		if ($questionTwo == $questionThree) {
			echo jsonreturn(['sign' => false, 'message' => '密保问题2和密保问题3不能相同！']);
			exit;
		}
		if ($answerOne == $answerTwo) {
			echo jsonreturn(['sign' => false, 'message' => '密保答案1和密保答案2不能相同！']);
			exit;
		}
		if ($answerOne == $answerThree) {
			echo jsonreturn(['sign' => false, 'message' => '密保答案1和密保答案3不能相同！']);
			exit;
		}
		if ($answerTwo == $answerThree) {
			echo jsonreturn(['sign' => false, 'message' => '密保答案2和密保答案3不能相同！']);
			exit;
		}
		$passwordpatten = '/^[\w\W]{4,16}$/';
		if (!preg_match($passwordpatten, $tradePwd)) {
			echo jsonreturn(['sign' => false, 'message' => '提款密码应为4-16位字符']);
			exit;
		}
		...
		if ($questoken) { //重置
			...
			if ($userinfo['question'] && is_array($userinfo['question'])) {
				echo jsonreturn(['sign' => false, 'message' => '您的密保已经设置无需重复设置']);
				exit;
			}
		}
		$apiparam['answerOne']          = $answerOne;
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/usersecurity', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
		session('questionanscheck', NULL);
	...
	function usereditdrawpass()
	{
		...
		if (!preg_match($passwordpatten, $oldpassword)) {
			echo jsonreturn(['sign' => false, 'message' => '旧提款密码应为4-16位字符']);
			exit;
		}
		if (!preg_match($passwordpatten, $password)) {
			echo jsonreturn(['sign' => false, 'message' => '新提款密码应为4-16位字符']);
			exit;
		}
		if ($password != $rpassword) {
			echo jsonreturn(['sign' => false, 'message' => '新提款密码两次输入不一致']);
			exit;
		}
		...
		if (md5(sha1($oldpassword)) != $userinfo['tradepassword']) {
			echo jsonreturn(['sign' => false, 'message' => '旧提款密码错误']);
			exit;
		}
		$apiparam = array();
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/usereditdrawpass', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
	...
	function usereditpass()
	{
		...
		if (!preg_match($passwordpatten, $oldpassword)) {
			echo jsonreturn(['sign' => false, 'message' => '旧密码应为6-16位字符']);
			exit;
		}
		if (!preg_match($passwordpatten, $password)) {
			echo jsonreturn(['sign' => false, 'message' => '新密码应为6-16位字符']);
			exit;
		}
		if ($password != $rpassword) {
			echo jsonreturn(['sign' => false, 'message' => '新密码两次输入不一致']);
			exit;
		}
		if (md5(sha1($oldpassword)) != $userinfo['password']) {
			echo jsonreturn(['sign' => false, 'message' => '旧密码错误']);
			exit;
		}
		$apiparam = array();
		$apiparam['oldpassword']      = $oldpassword;
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/usereditpass', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
	...
	function userbindrealname()
	{
		...
		if ($userinfo['tradepassword'] == '') {
			echo jsonreturn(['sign' => false, 'message' => '请先设置提款密码']);
			exit;
		}
		if ($userinfo['userbankname'] != '') {
			echo jsonreturn(['sign' => false, 'message' => '真实姓名绑定后不得修改']);
			exit;
		}
		$realnamepatten = '/^[\x80-\xff]{6,12}$/';
		$passwordpatten = '/^[\w\W]{4,16}$/';
		if (!preg_match($realnamepatten, $realname)) {
			echo jsonreturn(['sign' => false, 'message' => '真实姓名格式错误']);
			exit;
		}
		...
		$_api = new \Lib\api;
		$Result = [];
		$Result = $_api->sendHttpClient('Api/Account/userbindrealname', $apiparam);
		$return = [];
		$return['sign'] = $Result['sign'];
		$return['message'] = $Result['message'];
	...
	function userlevel()
	{
		...
		echo jsonreturn(['sign' => true, 'message' => '操作成功', 'data' => $data]);
		exit;
	}
	function userbets()
	{
	...
	function reportstatistics()
	{
		...
		if ($userinfo['proxy'] != 1) {
			echo jsonreturn(['sign' => false, 'message' => '您不是代理']);
			exit;
		}
	...
	function delsignuplink()
	{
		...
		$id = I('id', 0, 'intval');
		if (!$id) {
			echo jsonreturn(['sign' => false, 'message' => '开户链接不存在']);
			exit;
		}
		$sessionid = session('member_sessionid');
		$auth_id   = session('member_auth_id');
	...
	function addrecharge()
	{
		...
		if (!$paytype) {
			...
			$this->ajaxReturn(['sign' => false, 'message' => '请选择充值方式']);
			exit;
		}
		if ($amount <= 0) {
			$this->ajaxReturn(['sign' => false, 'message' => '充值金额不得低于0元']);
			exit;
		}
		if (in_array($paytype, ['alipay', 'tenpay', 'weixin', 'linepay']) && !$userpayname) {
			$this->ajaxReturn(['sign' => false, 'message' => '请输入您的支付账号']);
			exit;
		}
		$apiparam = array();
	...
	function isUserWithdrawLimit($apiparam = array())
	{
		...
		if (!$userinfo['userbankname']) {
			echo jsonreturn(['sign' => false, 'message' => '请先绑定真实姓名']);
			exit;
		}
		if (!$userinfo['banklist']) {
			echo jsonreturn(['sign' => false, 'message' => '请先绑定银行卡']);
			exit;
		}
		if ($userinfo['xima'] > 0) {
			echo jsonreturn(['sign' => false, 'message' => '打码不足，洗码金额为0时可以提款']);
			exit;
		}
		$apiparam['auth'] = ["member_auth_id" => $auth_id, "member_sessionid" => $sessionid];
		$_api = new \Lib\api;
	...
	function savetikuanorder()
	{
		...
		if ($_t < $tikuanstart) {
			echo json_encode(array('sign' => false, 'message' => '请在' . date('H:i', $tikuanstart) . '到' . date('H:i', $tikuanend) . '时间段之内提现，谢谢配合。'), JSON_UNESCAPED_UNICODE);
			exit;
		}
		if ($_t > $tikuanend) {
			echo json_encode(array('sign' => false, 'message' => '请在' . date('H:i', $tikuanstart) . '到' . date('H:i', $tikuanend) . '时间段之内提现，谢谢配合。'), JSON_UNESCAPED_UNICODE);
		...
		if ($count <= 0) {
			echo json_encode(['sign' => false, 'message' => '今日提现次达到了最大限次数！'], JSON_UNESCAPED_UNICODE);
			exit;
		}
		$bankid       = I('bankid');
		$amount       = I('amount');
	...
	function chatsent()
	{
		...
		if (strlen($context) < 1 || strlen($context) < 1 || strlen($userids) < 1) {
			echo jsonreturn(['sign' => false, 'message' => '缺少信件主题或内容或收件人', 'data' => ['islogin' => 0]]);
			exit;
		}
		$apiparam = array();

...
class ActivityController extends CommonController
{
	...
	function index()
	{
		...
		//晋级
		if (empty($jiangli) && $user['jinjijilu'] == 1) {   //如果为空则从普通会员算起,
	...
	function fanshui()
	{
		...
		if ($lastlqtime) {
			if ($_t - $lastlqtime <= 86400) { //一天领取
				$lqtype = 1;
			} elseif ($_t - $lastlqtime > 86400 && $_t - $lastlqtime < 86400 * 7) { //一周领取
				$lqtype = 2;

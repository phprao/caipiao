...
class XitongController extends Controller {
    ...
    function xtssc(){
		...
		if($_t<$_t1){
			...
			$actNo_t = (time()-strtotime(date('Y-m-d '.$cpinfo['closetime1'],$_t)))/$_expecttime;
		}
		...
		if($actNo<=20){
			if($actNo==1){
				$_openlist[] = [
					'expect'    => date("Ymd",strtotime("-1 day")) . str_pad($totalcount,$_length,0,STR_PAD_LEFT),
        ...
        if($cpinfo['closetime1'] == $cpinfo['closetime2'] && $actNo == 0){

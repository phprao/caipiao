...
class CaipiaoController extends CommonController
{
	...
	function yukaijiang()
	{
		...
		if ($cpinfo['closetime1'] == $cpinfo['closetime2']) {
			$totalcount = floor(86400 / $_expecttime);
		} else {
		...
		if ($_t < $_t1) {
			...
			$actNo_t = ($_t - strtotime(date('Y-m-d ' . $cpinfo['closetime1'], $_t))) / $_expecttime;
		}
		...
		if ($actNo > $totalcount) {
			...
			if ($actNo + 9 <= $totalcount) {
				for ($j = $actNo + 9, $jj = 0; $j >= $actNo; $j--, $jj++) {
					...
					if ($cpinfo['closetime1'] == $cpinfo['closetime2']) {
						$opentime  = date('Y-m-d H:i:s', strtotime($cpinfo['closetime2']) + 86400 - ($totalcount - $j) * $jgtime);
					} else {
				...
				for ($j = $totalcount, $jj = 0; $j >= $actNo; $j--, $jj++) {
					...
					if ($cpinfo['typeid'] == 'k3' or $cpinfo['typeid'] == 'keno') sort($rand_keys);
					if ($cpinfo['closetime1'] == $cpinfo['closetime2']) {
						$opentime  = date('Y-m-d H:i:s', strtotime($cpinfo['closetime2']) + 86400 - $jj * $jgtime);
					} else {
	...
	function editoldWfts()
	{
		...
		$id   = I('id');
		$remark   = I('remark');
		$int = M('wanfa_old')->where(['typeid' => $typeid, 'id' => $id])->setField(['remark' => $remark]);
		$int ? $this->success('操作成功') : $this->error('操作失败');
	}

	...
	function xtk3()
	{
		...
		if ($currentkj) {
			...
			if ($kjinfo = $ydb->where(['name' => $currentkj['name'], 'expect' => $currentkj['expect']])->find()) {
				echo "{$kjinfo['name']}-{$kjinfo['expect']}-{$kjinfo['opencode']}";
				exit;
			} else {
				...
				if ($setkjinfo) {
					if ($setkjinfo['config']['expect'] == $currentkj['expect']) {
						echo "{$setkjinfo['name']}-{$setkjinfo['config']['expect']}-{$setkjinfo['config']['opencode']}";
						exit;
					} else {
						$int = M('setting')->where(['name' => $currentkj['name']])->setField(['value' => $data['value']]);
						if ($int) {
							echo "{$data['name']}-{$currentkj['expect']}-{$currentkj['opencode']}";
							exit;
						} else {
							echo "---1--";
							exit;
